...
void transform(float *img, size_t stride, float* buf)
{
    ...
    __m256 tmp0 = _mm256_loadu_ps(img + 0*8) * _mm256_set1_ps(0.353553390593274f);
    __m256 tmp1 = _mm256_loadu_ps(img + 2*8) * _mm256_set1_ps(0.461939766255643f);
    __m256 tmp2 = _mm256_loadu_ps(img + 4*8) * _mm256_set1_ps(0.353553390593274f);
    __m256 tmp3 = _mm256_loadu_ps(img + 6*8) * _mm256_set1_ps(0.191341716182545f);
...
/* phases 5-3 */
    __m256 tmp12 = (tmp1 - tmp3) * _mm256_set1_ps(M_SQRT2) - tmp13; /* 2*c4 */
...
/* phase 2 */
    ...
    __m256 tmp4 = _mm256_loadu_ps(img + 1*8) * _mm256_set1_ps(0.490392640201615f);
    __m256 tmp5 = _mm256_loadu_ps(img + 3*8) * _mm256_set1_ps(0.415734806151273f);
    __m256 tmp6 = _mm256_loadu_ps(img + 5*8) * _mm256_set1_ps(0.277785116509801f);
    __m256 tmp7 = _mm256_loadu_ps(img + 7*8) * _mm256_set1_ps(0.097545161008064f);
...
/* phase 3 */
    ...
    tmp13 = tmp1 + tmp3;
/* phases 5-3 */
    tmp12 = (tmp1 - tmp3) * _mm256_set1_ps(M_SQRT2) - tmp13; /* 2*c4 */
...
//  which is
    ...
    _mm256_storeu_ps(buf + 0*stride, i0 + _mm256_loadu_ps(buf + 0*stride));
    _mm256_storeu_ps(buf + 1*stride, i1 + _mm256_loadu_ps(buf + 1*stride));
    _mm256_storeu_ps(buf + 2*stride, i2 + _mm256_loadu_ps(buf + 2*stride));
    _mm256_storeu_ps(buf + 3*stride, i3 + _mm256_loadu_ps(buf + 3*stride));
    _mm256_storeu_ps(buf + 4*stride, i4 + _mm256_loadu_ps(buf + 4*stride));
    _mm256_storeu_ps(buf + 5*stride, i5 + _mm256_loadu_ps(buf + 5*stride));
    _mm256_storeu_ps(buf + 6*stride, i6 + _mm256_loadu_ps(buf + 6*stride));
    _mm256_storeu_ps(buf + 7*stride, i7 + _mm256_loadu_ps(buf + 7*stride));
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    memset(vout, 0, sizeof(float)*dimo[0]*dimo[1]*pics);

    int istri = dimi[0]/deci;
    int jstri = dimi[0]*dimi[1]/deci;
    int kstri = dimi[0]*dimi[1]*dimi[2];

    int istro = 1;
    int jstro = dimo[0];
    int kstro = dimo[0]*dimo[1];

    for (int k = 0; k < pics; k++)
    {
        #pragma omp parallel for schedule(static, 16)
	for (int j = 0; j < dimo[1]-7; j+=deci)
	{
	    for (int i = 0; i < dimo[0]-7; i+=deci)
	    {
		transform(vin1+k*kstri+j*jstri+i*istri,
			  dimo[0],vout+k*kstro+j*jstro+i);
	    }
	}
    }
    return;
